...
export class RaffleTicketsService {
  ...
  findTicketsByRaffleId(id: number): Observable<RaffleTicketDTO[]> {
    return this.http
      .get<BoletoDTO[]>(`${environment.api}/boletos/sorteo/${id}`)
      .pipe(
  ...
  reserveTicket(ticketId: number): Observable<RaffleTicketDTO> {
    const user = this.auth.getUser();
    return this.http
      .put<BoletoDTO>(`${environment.api}/boletos/apartado/query`, null, {
        params: <any>{ idBoleto: ticketId, correo: user.correo },
      })
      .pipe(
  ...
  freeTicket(ticketId: number): Observable<RaffleTicketDTO> {
    return this.http
      .put<BoletoDTO>(`${environment.api}/boletos/libre/${ticketId}`, null)
      .pipe(
        map((boleto: BoletoDTO) => ({
          id: boleto.id,
          ticketNumber: boleto.numeroBoleto,
  ...
  buyTicket(ticketId: number): Observable<RaffleTicketDTO> {
    const user = this.auth.getUser();
    return this.http
      .put<BoletoDTO>(`${environment.api}/boletos/vendido/query`, null, {
        params: <any>{ idBoleto: ticketId, correo: user.correo },
      })
      .pipe(
        map((boleto: BoletoDTO) => ({
